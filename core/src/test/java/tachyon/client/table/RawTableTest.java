...
public class RawTableTest {
  ...
  public void getColumnsTest() throws IOException {
    for (int k = 1; k < CommonConf.get().MAX_COLUMNS; k += CommonConf.get().MAX_COLUMNS / 5) {
      int fileId = mTfs.createRawTable(new TachyonURI("/table" + k), k);
      RawTable table = mTfs.getRawTable(fileId);
      Assert.assertEquals(k, table.getColumns());
      table = mTfs.getRawTable(new TachyonURI("/table" + k));
      Assert.assertEquals(k, table.getColumns());

      fileId =
          mTfs.createRawTable(new TachyonURI("/tabl" + k), k,
              TestUtils.getIncreasingByteBuffer(k % 10));
      table = mTfs.getRawTable(fileId);
      Assert.assertEquals(k, table.getColumns());
      table = mTfs.getRawTable(new TachyonURI("/tabl" + k));
      Assert.assertEquals(k, table.getColumns());
    }
  }
  ...
  public void getIdTest() throws IOException {
    for (int k = 1; k < CommonConf.get().MAX_COLUMNS; k += CommonConf.get().MAX_COLUMNS / 5) {
      int fileId = mTfs.createRawTable(new TachyonURI("/table" + k), 1);
      RawTable table = mTfs.getRawTable(fileId);
      ...
      fileId =
          mTfs.createRawTable(new TachyonURI("/tabl" + k), 1,
              TestUtils.getIncreasingByteBuffer(k % 10));
      table = mTfs.getRawTable(fileId);
      Assert.assertEquals(fileId, table.getId());
      table = mTfs.getRawTable(new TachyonURI("/tabl" + k));
  ...
  public void getMetadataTest() throws IOException {
    for (int k = 1; k < CommonConf.get().MAX_COLUMNS; k += CommonConf.get().MAX_COLUMNS / 5) {
      int fileId = mTfs.createRawTable(new TachyonURI("/x/table" + k), 1);
      RawTable table = mTfs.getRawTable(fileId);
      Assert.assertEquals(ByteBuffer.allocate(0), table.getMetadata());
      Assert.assertEquals(ByteBuffer.allocate(0), table.getMetadata());
      table = mTfs.getRawTable(new TachyonURI("/x/table" + k));
      Assert.assertEquals(ByteBuffer.allocate(0), table.getMetadata());

      fileId =
          mTfs.createRawTable(new TachyonURI("/y/tab" + k), 1,
              TestUtils.getIncreasingByteBuffer(k % 7));
      table = mTfs.getRawTable(fileId);
      Assert.assertEquals(TestUtils.getIncreasingByteBuffer(k % 7), table.getMetadata());
      Assert.assertEquals(TestUtils.getIncreasingByteBuffer(k % 7), table.getMetadata());
      table = mTfs.getRawTable(new TachyonURI("/y/tab" + k));
      Assert.assertEquals(TestUtils.getIncreasingByteBuffer(k % 7), table.getMetadata());
      Assert.assertEquals(TestUtils.getIncreasingByteBuffer(k % 7), table.getMetadata());
    }
  }
